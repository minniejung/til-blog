...
➡ Uniswap V2의 TWAP은 DeFi 오라클의 안전성과 신뢰도를 높인 대표적 사례입니다.`}</pre>
	...
	},
	{
		id: 13,
		date: '07/05/2025',
		tags: ['Oracle', 'Smart Contract', 'Blockchain'],
		title: 'Oracle',
		content: (
			<div>
				<h3>스마트 컨트랙트의 한계</h3>
				<pre>{`스마트 컨트랙트는 블록체인이라는 폐쇄된 환경에서 실행됩니다.
즉, 블록체인 외부에 존재하는 데이터에 직접 접근할 수 없습니다.
...
이런 정보는 블록체인에 저장되어 있지 않기 때문에,
스마트 컨트랙트만으로는 알 수 없습니다.
이는 블록체인의 보안성과 결정성을 유지하기 위한 설계 원칙입니다.`}</pre>

				<h3>오라클이 필요한 이유</h3>
				<pre>{`외부 데이터를 참조해야 하는 많은 서비스들이 있습니다.

    예시:
    - DeFi에서는 실시간 자산 가격이 필요하고,
    - 날씨 보험 서비스는 실제 기상 정보가 필요하며,
    - 게임은 외부 난수 발생기(randomness)에 의존합니다.

이처럼 스마트 컨트랙트가 외부 현실을 인식해야 할 때,
중간에서 데이터를 안전하게 전달해주는 존재가 바로 오라클입니다.`}</pre>

				<h3>오라클(Oracle)의 정의</h3>
				<pre>{`오라클은 블록체인 외부의 데이터를 읽어와,
스마트 컨트랙트에게 전달하는 중개 시스템입니다.

즉, 오라클은 블록체인과 현실 세계를 연결하는 '데이터 브릿지'입니다.`}</pre>

				<h3>오라클의 주요 역할</h3>
				<pre>{`• 데이터 공급:
  - 외부 API, 센서, 서버에서 데이터를 받아 블록체인에 전달

• 트리거 제공:
  - 계약 조건을 만족시키는 외부 이벤트를 감지하고 알려줌

• 자동화 지원:
  - 외부 정보를 기반으로 계약 실행을 자동화`}</pre>
			</div>
		),
	},
	{
		...
		content: (
			<div>
				<h3>오라클의 동작 방식</h3>
				<pre>{`오라클은 외부 데이터를 블록체인에 전달하는 방법에 따라 두 가지 방식으로 나뉩니다.`}</pre>

				<h3>1. 푸시 방식 (Push)</h3>
				<pre>{`• 오라클이 일정 주기로 데이터를 블록체인에 자동 기록합니다.
• 예: 10초마다 ETH/USD 가격을 업데이트
    ...
    단점:
    - 필요 없는 정보도 지속적으로 기록됨 → 가스비 낭비`}</pre>

				<h3>2. 풀 방식 (Pull)</h3>
				<pre>{`• 스마트 컨트랙트가 특정 시점에 오라클에 데이터를 요청합니다.
• 예: getWeatherData() 호출 시 오라클이 날씨 정보를 제공
    ...
    단점:
    - 응답 지연 가능성 있음
    - 요청 → 응답에 트랜잭션 2개 필요`}</pre>

				<h3>오라클의 신뢰 구조</h3>
				<pre>{`오라클은 데이터를 어떻게 신뢰성 있게 전달하느냐에 따라 중앙화형과 탈중앙화형으로 나뉩니다.`}</pre>

				<h3>1. 중앙화 오라클</h3>
				<pre>{`• 하나의 서버 또는 기관이 데이터를 제공합니다.
• 예: 특정 회사의 환율 API
    ...
    - 탈중앙화와 철학적으로 어긋남`}</pre>
				...
				<h3>2. 탈중앙 오라클</h3>
				<pre>{`• 여러 노드가 데이터를 수집하고, 합의를 통해 하나의 값을 생성합니다.
• 예: 21개 노드의 평균값을 제출
    ...
    - 높은 운영 비용`}</pre>
				...
				<pre>{`• Chainlink
  - 구조: 탈중앙 / 푸시 + 풀 지원
  - 특징: 가장 보편적인 오라클, 다양한 데이터 피드 제공, VRF(Randomness) 지원

• Band Protocol
  - 구조: 탈중앙 / 푸시 중심
  - 특징: Cosmos 기반, 빠른 응답성, IBC 지원

• Tellor
  - 구조: 탈중앙 / 풀 방식
  - 특징: 채굴형 오라클, 요청이 있을 때 값 제공

• UMA
  - 구조: 탈중앙 / 풀 방식
  - 특징: Optimistic Oracle → 값에 이의 없으면 자동 채택, 분쟁 발생 시 투표로 해결`}</pre>
			</div>
		),
	},
	{
		...
		content: (
			<div>
				<h3>오라클이 필요한 주요 사용 사례</h3>
				<pre>{`스마트 컨트랙트는 외부 데이터를 기반으로 조건을 실행해야 할 때 오라클이 필요합니다.
대표적인 사례들을 살펴보면 다음과 같습니다:`}</pre>

				<h3>1. 가격 정보 (암호화폐, 주식, 환율 등)</h3>
				<pre>{`• DeFi에서 가장 많이 활용되는 오라클 분야입니다.
• 담보 대출, 스왑, 자동 청산 등을 위해 실시간 가격 데이터가 필요합니다.

    사용 예:
    - AAVE, MakerDAO: 담보가치 평가
    - Uniswap: 토큰 교환 비율 계산

    오라클:
    - Chainlink Price Feed (ETH/USD, BTC/DAI 등)`}</pre>

				<h3>2. 날씨 데이터</h3>
				<pre>{`• 조건부 보험 계약, 농업 보상 등에 사용됩니다.

    사용 예:
    - 비가 일정량 이상 내리면 자동 보험 지급
    - 항해 중 악천후 발생 시 보험 트리거

    오라클:
    - 외부 날씨 API (OpenWeather 등)
    - Chainlink External Adapter`}</pre>

				<h3>3. 스포츠 경기 결과</h3>
				<pre>{`• 베팅 결과 정산, 팬 이벤트 등 자동화에 사용됩니다.
    ...
    오라클:
    - 스포츠 API → Tellor 또는 UMA(Optimistic Oracle)`}</pre>

				<h3>4. 무작위값(Randomness)</h3>
				<pre>{`• 게임, 추첨, NFT 속성 부여 등에서 조작 불가능한 랜덤값이 필요합니다.
    ...
    - requestRandomness() 호출 → fulfillRandomWords()로 응답`}</pre>

				<h3>5. 외부 API 연결</h3>
				<pre>{`• 항공편, 물류, 교통, 의료 등 IoT/실시간 정보에 연동됩니다.
    ...
    - Node.js 서버에서 API 호출 후 오라클 노드 → 스마트 컨트랙트 전달`}</pre>

				<h3>요약 테이블</h3>
				<pre>{`사용 사례         | 목적                    | 오라클 예시
------------------------------------------------------------------
가격 정보         | 환율, 담보 평가         | Chainlink Price Feeds
날씨             | 보험, 보장 조건         | 외부 API + Chainlink
경기 결과         | 베팅, 리워드            | Sports API + Tellor/UMA
무작위값          | 게임, 추첨, NFT        | Chainlink VRF
API 연결         | 현실 조건 자동화         | Chainlink External Adapter`}</pre>

				<pre>{`오라클은 스마트 컨트랙트와 현실 세계를 연결하는 핵심 요소입니다.
단순한 가격 정보에서부터 날씨, 스포츠, 무작위값, 항공편 API까지 다양한 정보와 조건을 온체인으로 가져올 수 있도록 해줍니다.`}</pre>
			</div>
		),
	},
	{
		...
		content: (
			<div>
				<h3>오라클 문제 (Oracle Problem)란?</h3>
				<pre>{`스마트 컨트랙트는 블록체인 위에서 결정적이고 안전하게 실행되지만,
외부 데이터를 가져오는 오라클은 블록체인 밖에서 작동합니다.

결과적으로, 컨트랙트가 신뢰할 수 없는 외부 정보에 의존하게 되어
전체 로직이 잘못된 결과를 낼 수 있는 위험이 생깁니다.

이 근본적인 한계를 오라클 문제(Oracle Problem)라고 부릅니다.`}</pre>
				<h3>주요 위협 요소</h3>
				<h3>1. 조작 가능성</h3>
				<pre>{`• 단일 오라클이 데이터를 조작하거나 해킹당할 수 있습니다.

    예:
    - 가격을 조작해 담보 대출 강제 청산
    - NFT 민팅 결과나 추첨 당첨자 조작`}</pre>
				<h3>2. 랜덤값 조작 문제</h3>
				<pre>{`• 스마트 컨트랙트 내에서 block.timestamp나 keccak256로 만든 랜덤값은 예측이 가능합니다.`}</pre>
...
// 공격자는 입력값이나 블록 타이밍을 조절해 결과를 통제할 수 있음

// 해결책:
// - Chainlink VRF (Verifiable Random Function)
// - 암호학적 서명 포함 → 무작위값을 컨트랙트가 검증 가능`}</SyntaxHighlighter>
				<h3>3. 시간 지연</h3>
				<pre>{`• 오라클이 데이터를 수집하고 블록체인에 반영하는 데 시간이 소요될 수 있습니다.

    원인:
    - 네트워크 지연, 블록 혼잡, 오라클 노드 오프라인 등

    결과:
    - 오래된 데이터로 트랜잭션 실행
    - 스왑 가격 오류, 시간 제한 로직 실패`}</pre>
				<h3>4. 가용성 문제</h3>
				<pre>{`• 오라클이 다운되면 스마트 컨트랙트는 외부 정보를 받지 못하고 멈추게 됩니다.

    결과:
    - DeFi, 보험, 게임 등 실시간 반응이 필요한 서비스 마비`}</pre>
				<h3>해결 전략</h3>
				<h3>1. 다중 오라클 사용</h3>
				<pre>{`• 여러 노드의 데이터를 사용해 단일 실패에 대비

    예:
    - Chainlink는 7~31개 노드 데이터를 집계해 제공`}</pre>
				<h3>2. Aggregation (집계)</h3>
				<pre>{`• 중앙값, 평균값, 합의 알고리즘 등을 통해 데이터 왜곡을 방지

    예:
    - 5개 중 1개가 잘못된 값을 주더라도 중앙값으로 정확한 결과 도출`}</pre>
				<h3>3. 탈중앙화 오라클 네트워크</h3>
				<pre>{`• 오픈된 다수의 노드가 인센티브 기반으로 데이터 제공

대표 예:
- Chainlink, Tellor, UMA`}</pre>
				<h3>요약</h3>
				<pre>{`📌 오라클 관련 보안 이슈 및 해결 방안

1. 오라클 문제
- 설명: 블록체인은 안전하지만 외부 데이터는 신뢰할 수 없음
- 해결 방안: 신뢰 가능한 오라클 사용

2. 조작 위험
- 설명: 단일 오라클이 데이터를 조작하거나 해킹당할 수 있음
- 해결 방안: 다중 오라클 사용 + Aggregation(중앙값/평균값 등)

3. 무작위값 조작
- 설명: block.timestamp 등 내부 랜덤 값은 예측 가능
- 해결 방안: 외부 VRF 사용 (예: Chainlink VRF)

4. 시간 지연
- 설명: 오라클 응답이 늦어 트랜잭션 실패 또는 시차 오류 발생
- 해결 방안: 예측 기반 설계 + Fallback 로직 구현

5. 가용성 문제
- 설명: 오라클이 다운되면 컨트랙트도 멈추는 상황 발생
- 해결 방안: 탈중앙화된 오라클 네트워크 사용 (Chainlink, Tellor 등)`}</pre>
				<pre>{`오라클은 스마트 컨트랙트의 현실 연결을 가능하게 하지만,
데이터의 신뢰성, 조작 방지, 반응 속도는 모두 설계 시 핵심 고려 요소입니다.

특히 무작위값처럼 조작 위험이 높은 요소는 Chainlink VRF 같은 암호학적 방식으로 처리하는 것이 필수입니다.`}</pre>
			</div>
		),
	},
	{
		id: 17,
		date: '07/05/2025',
		tags: ['Chainlink VRF', 'Oracle', 'Smart Contract', 'Blockchain'],
		title: 'Chainlink VRF',
		content: (
			<div>
				<h3>스마트 컨트랙트에서 랜덤 값이 어려운 이유</h3>
				<pre>{`- 블록체인은 결정적인 시스템이기 때문에 모든 노드가 같은 결과를 생성해야 함
- block.timestamp, block.number, block.prevrandao 등은 예측 가능
- 블록 생성자가 원하는 결과를 유도할 수 있음`}</pre>

				<h3>Chainlink VRF란?</h3>
				<pre>{`- Verifiable Random Function (검증 가능한 무작위 함수)
- 암호학적으로 안전한 무작위 값을 생성하고, 서명과 함께 제공
- 스마트 컨트랙트는 그 무작위 값이 조작되지 않았는지 검증 가능`}</pre>

				<h3>동작 흐름</h3>
				<pre>{`- 스마트 컨트랙트가 Chainlink VRF에 무작위 값을 요청
- 오라클 노드가 무작위 값과 증명 생성
- fulfillRandomWords() 함수를 통해 무작위 값 전달
- 스마트 컨트랙트는 무작위 값과 서명을 검증 후 사용`}</pre>

				<h3>보안적 특징</h3>
				<pre>{`- 예측 불가: 요청 전에는 아무도 결과를 알 수 없음
- 조작 불가: 노드가 결과를 조작하면 서명 검증에 실패함
- 검증 가능: 스마트 컨트랙트에서 자동 검증 수행
- 탈중앙화: 여러 노드가 무작위 값을 생성해 보안성 강화`}</pre>

				<h3>활용 예시</h3>
				<pre>{`- NFT 민팅 시 랜덤 속성 부여
- 온체인 게임의 카드 뽑기, 전투 결과 등
- 추첨 시스템에서 무작위 당첨자 선정
- 로또 시스템에서 무작위 번호 생성`}</pre>

				<h3>Pull 방식 구조</h3>
				<pre>{`- 사용자가 requestRandomWords()를 호출하여 요청
- Chainlink 노드가 응답을 준비
- fulfillRandomWords() 함수가 무작위 값을 전달하고 스마트 컨트랙트는 수신 및 검증`}</pre>

				<h3>✔️ 결론</h3>
				<pre>{`Chainlink VRF는 무작위성 요구가 중요한 DeFi, NFT, 게임 등에서
공정하고 신뢰할 수 있는 랜덤 값을 제공하는 필수 오라클입니다.`}</pre>
			</div>
		),
	},
]
